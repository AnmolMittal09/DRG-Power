    ...
    <div className="pt-56 md:pt-64 lg:pt-72 pb-16 md:pb-32 bg-white">
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center mb-24 md:mb-48">
          <div className="space-y-8 md:space-y-12">
            <div>
              ...
              <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-[8rem] font-bold tracking-tight text-slate-900 leading-[0.9] italic uppercase">The Energy <br /> Umbrella.</h1>
            </div>
            ...
            <div className="flex gap-10 md:gap-16 pt-4 md:pt-8 border-t border-slate-50">
              ...
              <div>
                ...
                <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.4em] text-slate-400">Group Legacy</span>
              </div>
            </div>
          </div>
          <div className="relative mt-8 lg:mt-0">
             <div className="absolute -inset-8 md:-inset-16 bg-red-600/5 blur-[80px] md:blur-[150px] rounded-full opacity-60" />
             <div className="relative rounded-[2rem] md:rounded-[4rem] overflow-hidden shadow-3xl border border-slate-100 bg-slate-100">
               <img 
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 mb-24 md:mb-48">
          <div className="bg-slate-900 text-white p-8 sm:p-12 md:p-20 rounded-[2rem] md:rounded-[4rem] shadow-3xl relative overflow-hidden group border border-white/5">
          ...
          <div className="bg-slate-50 p-8 sm:p-12 md:p-20 rounded-[2rem] md:rounded-[4rem] shadow-xl relative overflow-hidden group border border-slate-100">
            <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-8 tracking-tighter uppercase italic text-slate-900">The Vision</h2>
            <p className="text-lg sm:text-xl md:text-3xl text-slate-500 font-medium leading-tight italic relative z-10">
              {VISION}
            </p>
            <div className="absolute -bottom-8 -right-8 md:-bottom-16 md:-right-16 text-[10rem] md:text-[18rem] font-black text-slate-900/5 select-none pointer-events-none">V</div>
          </div>
        </div>
        ...
        <div className="mb-24 md:mb-48">
          <div className="text-center mb-12 md:mb-24">
            ...
            <h2 className="text-4xl md:text-6xl font-bold tracking-tight text-slate-900 italic uppercase">The Technical Core.</h2>
            <p className="text-base md:text-xl text-slate-400 mt-4 md:mt-6 max-w-2xl mx-auto font-medium italic">
              Blending technical expertise with strategic vision toward sustainable growth.
            </p>
          </div>
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 max-w-6xl mx-auto">
            {LEADERSHIP.map((member, i) => (
              <div key={i} className="bg-white p-8 sm:p-12 md:p-16 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-2xl hover:shadow-red-900/5 transition-all group">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 md:gap-8 mb-8 md:mb-10">
                   <div className="w-16 h-16 md:w-24 md:h-24 bg-slate-900 rounded-2xl flex items-center justify-center text-white text-2xl md:text-4xl shadow-xl group-hover:rotate-6 transition-transform shrink-0">
                      {member.name.charAt(0)}
                   </div>
                   <div>
                      <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter uppercase leading-none">{member.name}</h3>
        ...
        <div className="text-center mb-12 md:mb-24">
          ...
          <h2 className="text-4xl md:text-6xl font-bold tracking-tight text-slate-900 italic uppercase">Foundation Pillars.</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-px bg-slate-100 border border-slate-100 rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl mb-16 md:mb-32">
          {CORE_VALUES.map((v, i) => (
            <div key={i} className="bg-white p-8 sm:p-10 md:p-14 text-center hover:bg-slate-50 transition-all duration-500 group">
              <div className="text-5xl md:text-6xl mb-6 md:mb-10 group-hover:scale-110 transition-transform">{v.icon}</div>
              <h3 className="text-[10px] md:text-sm font-black mb-4 md:mb-6 tracking-[0.2em] leading-tight uppercase text-slate-900">{v.title}</h3>
