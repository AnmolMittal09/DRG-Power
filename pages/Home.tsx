    ...
    <section className="relative min-h-screen flex flex-col justify-center bg-[#050505] overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10 pt-56 md:pt-64 lg:pt-80 pb-16 lg:pb-20">
        <div className="grid lg:grid-cols-12 gap-8 md:gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-7 space-y-8 md:space-y-12 animate-in fade-in slide-in-from-left-12 duration-1000">
            <div className="space-y-6 md:space-y-8">
              <div className="inline-flex items-center gap-3 bg-white/5 backdrop-blur-md border border-white/10 px-4 md:px-6 py-1.5 md:py-2 rounded-full">
                <span className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-[#BE1E2D] animate-pulse"></span>
                <span className="text-white text-[7px] md:text-[9px] font-black uppercase tracking-[0.4em]">{TAGLINE}</span>
              </div>
              
              <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-[7.5rem] font-black text-white leading-[0.9] lg:leading-[0.85] tracking-tighter uppercase italic">
                ONE SOLAR <br />
                <span className="text-[#BE1E2D]">PARTNER.</span> <br />
                <span className="text-2xl sm:text-4xl md:text-6xl lg:text-7xl block mt-2 md:mt-4 text-white/90">COMPLETE SOLUTIONS.</span>
              </h1>
              
              <p className="text-base sm:text-lg md:text-2xl text-slate-400 max-w-xl font-medium leading-relaxed italic border-l-4 border-[#BE1E2D] pl-4 md:pl-8 py-1 md:py-2">
            ...
            <div className="flex flex-wrap gap-4 md:gap-6 items-center">
              <a 
                ...
                className="group relative inline-flex items-center gap-3 md:gap-4 px-8 md:px-12 py-4 md:py-6 bg-[#BE1E2D] hover:bg-[#a11a27] text-white text-base md:text-lg font-black rounded-xl transition-all shadow-[0_20px_50px_-10px_rgba(190,30,45,0.6)] hover:shadow-[0_30px_60px_-15px_rgba(190,30,45,0.8)] hover:-translate-y-1 active:scale-95 uppercase tracking-widest"
              >
                Get A Quote
              ...
              <Link 
                to="/about"
                className="inline-flex items-center gap-3 md:gap-4 px-8 md:px-12 py-4 md:py-6 bg-white/5 hover:bg-white/10 text-white text-base md:text-lg font-black rounded-xl border border-white/10 transition-all uppercase tracking-widest backdrop-blur-sm"
              >
                Why DRG?
              </Link>
            </div>

            <div className="flex gap-10 md:gap-16 pt-8 md:pt-12 border-t border-white/5">
              <div className="space-y-1 md:space-y-2">
                <span className="block text-3xl md:text-5xl font-black text-white italic tracking-tighter">25+</span>
                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-[#BE1E2D]">Years Warranty</span>
              </div>
              <div className="space-y-1 md:space-y-2">
                <span className="block text-3xl md:text-5xl font-black text-white italic tracking-tighter">92k+</span>
                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-[#BE1E2D]">Watts Saved</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative animate-in fade-in zoom-in duration-1000 delay-300">
            <div className="relative rounded-[2rem] md:rounded-[3.5rem] overflow-hidden shadow-2xl border border-white/5 group aspect-[4/5] bg-slate-900">
              <img 
              ...
              <div className="absolute top-4 md:top-8 right-4 md:right-8 bg-[#00AEEF]/10 backdrop-blur-3xl border border-white/10 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2rem] text-right shadow-3xl transform hover:scale-105 transition-transform cursor-default">
                <span className="block text-white/60 text-[7px] md:text-[9px] font-black uppercase tracking-widest mb-0.5 md:mb-1">Grid Efficiency</span>
                <span className="block text-xl md:text-3xl font-black text-[#00AEEF] italic tracking-tighter">99.8%</span>
              </div>
              
              <div className="absolute bottom-6 md:bottom-12 left-6 md:left-12">
                <p className="text-white font-black text-xs md:text-sm uppercase tracking-[0.2em] italic">Infrastructure Alpha</p>
                <div className="h-0.5 md:h-1 w-8 md:w-12 bg-[#BE1E2D] mt-1 md:mt-2"></div>
              </div>
            </div>
            
            {/* Ambient Background Blur for depth */}
            <div className="absolute -top-12 -right-12 w-[250px] md:w-[350px] h-[250px] md:h-[350px] bg-[#BE1E2D]/10 blur-[100px] md:blur-[120px] rounded-full pointer-events-none animate-pulse"></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/80 backdrop-blur-2xl border-t border-white/5 mt-auto">
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex flex-wrap items-center justify-around md:justify-between py-6 md:py-10 gap-4 md:gap-8">
            {[
              ...
              <div key={i} className="flex items-center gap-2 md:gap-4 group cursor-default">
                <span className="text-xl md:text-2xl grayscale group-hover:grayscale-0 transition-all duration-300 opacity-40 group-hover:opacity-100">{item.icon}</span>
                <span className="text-[7px] md:text-[10px] font-black text-white/30 group-hover:text-white uppercase tracking-[0.3em] md:tracking-[0.4em] transition-all">{item.label}</span>
              </div>
            ))}
          </div>
        </div>
