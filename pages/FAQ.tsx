          ...
          {FAQS.map((faq, i) => (
            <div key={i} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              >
                ...
                <span className={`text-2xl transition-transform duration-300 font-mono ${activeIndex === i ? 'rotate-180 text-green-600' : 'text-slate-400'}`}>
                  {activeIndex === i ? '−' : '+'}
                </span>
              </button>
              {activeIndex === i && (
                <div className="p-6 pt-0 text-slate-600 leading-relaxed animate-in slide-in-from-top-2 duration-200 font-medium">
                  {faq.answer}
                </div>
              )}
            </div>
